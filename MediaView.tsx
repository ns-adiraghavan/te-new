        ...
        {activeTab === "Impact Stories" && (
          <div style={{ columns: 2, columnGap: 20 }}>
            {IMPACT_STORIES.map((s) => {
              const wordCount = [s.openingPara, ...s.sections.map(sec => [sec.heading ?? "", sec.body ?? "", ...(sec.bullets ?? [])].join(" "))].join(" ").split(/\s+/).filter(Boolean).length;
                  ...
                  <div style={{ padding: "20px 22px 22px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 500 }}>{readMins} min read</span>
